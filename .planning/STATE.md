...
- [04-01]: Migration env.py imports app.models so Base.metadata sees all tables for autogenerate
- [04-01]: Removed SQLite-incompatible alter_column(tasks.priority) from autogenerated migration — VARCHAR(10) stores enum values correctly
- [04-01]: CalendarSync uses single-row pattern: one row holds credentials_json, sync_token, last_synced_at
- [04-01]: Wave 0 tests defer imports of unimplemented modules to test bodies (not module-level) to get clean RED failures not collection errors
...
Last session: 2026-06-12T22:45:00Z
Next action: Completed 04-01 (calendar sync foundation: Google libs, models, migration, Wave 0 RED tests); ready for 04-02
